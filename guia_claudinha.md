...
| `portfolio_photos` | fotos base64 do prestador, gerais ou vinculadas a um serviço. Não entra no load inicial; é buscada sob demanda por prestador. Coluna `kind` (2026-08-30, migration `20260830030000_portfolio_photo_kind.sql`): `'foto'` (padrão) ou `'banner'` — banners são a faixa larga em destaque no topo da vitrine (`.publicBanners`), fotos gerais são a galeria (`.publicGallery`); ambas upload na aba Vitrine, `uploadPortfolioPhoto(serviceId, file, kind)`. |
...
- **Receita pra próxima vez que precisar ver uma tela autenticada sem ter a senha de ninguém**: função Edge temporária com `service_role` + `x-debug-token` fixo no código (não checar `is_master_admin()`, já que não tem sessão nenhuma pra checar) → criar usuário de teste → vincular via tabela com RLS aberta pro `anon` (ou outra function, se a tabela não for `anon`-aberta) → testar → **sempre limpar tudo depois** (deletar usuário via a mesma function com uma `action: 'delete'`, desvincular, `supabase functions delete`, apagar o arquivo local). Pra ver telas com dado de verdade (ex.: Agenda com agendamentos), inserir linhas de teste direto via `anon` key nas tabelas relevantes (`bookings`, etc.) antes de logar, e apagar tudo no final junto com a conta.
- **Botões de ação compactados (mesma rodada)**: em Serviços e Recursos, Subir/Descer/Remover tinham ícone+texto lado a lado, repetindo em cada linha — pesado visualmente com vários itens. Viraram ícone-só com `title`/`aria-label` (Pausar/Ativar manteve texto, é estado que vale ler). `.compactButton` ganhou `justify-self: start` porque esticava largura total dentro do grid de 1 coluna do `.inviteEditor`.

**5.12 Agenda mais dinâmica (2026-08-30)**: Jefferson pediu "mais dinâmico" pra gestão de agenda. Testando ao vivo com agendamentos reais (pendente/confirmado/concluído no mesmo dia), o problema ficou óbvio: a mesma informação aparecia duas vezes — na grade do dia (`.scheduleBoard`, só leitura, status como texto plano) e na lista detalhada mais embaixo (`.appointmentList`, com os `statusPills` de verdade) — pra mudar status de algo que você via na grade, tinha que rolar até achar a linha equivalente na lista. Dois ajustes:
- **Ação rápida direto no card da grade**: cada `slotCard` com `slot.booking` agora mostra um botão de avanço (`Confirmar` se `pendente`, `Concluir` se `confirmado`, nenhum botão se já for estado terminal `concluido`/`cancelado`) chamando o mesmo `updateBookingStatus` de sempre — não é um controle novo, é o mesmo dado/função só exposto num segundo lugar mais conveniente pro caso comum. A lista detalhada embaixo continua existindo pra ações menos comuns (cancelar, reverter status).
- **Navegação por dia**: setas `ChevronLeft`/`ChevronRight` (`shiftAgendaDate(±1)`, soma/subtrai dia com `Date.setDate`) ao lado do `<input type="date">`, mais um botão "Hoje" que só aparece quando `agendaDate` é diferente do dia atual — evita reabrir o calendário nativo pra andar dia a dia.
...
**Página pública isolada (2026-08-25)**: quando `session.role === 'cliente' && publicProviderId` (visitante chegou por um link `#loja=`/`#agendar=` de um prestador específico), a app renderiza um `<main className="publicStorefront">` totalmente à parte — **sem** a sidebar/topbar internos, sem stats da plataforma, sem nada que sugira que existem outros prestadores. Motivo: a versão anterior reaproveitava o mesmo shell do painel interno pra essa página, e chegou a vazar contagem de "Prestadores"/"Clientes" da plataforma inteira numa página que devia ser 100% do prestador — feedback direto do Jefferson ("como pode ser um portfólio se aparece sinais de outra loja"). Esse branch isolado fica logo antes do `return` principal em `App.jsx` (`if (session.role === 'cliente' && publicProviderId && bookingService) { ... }`) e reaproveita as mesmas variáveis já computadas (`bookingService`, `filteredServices`, `createBooking` etc.) — só muda o wrapper visual.

**5.13 Página pública unificada + modelo de e-commerce no agendamento (2026-08-30)**: essa seção existia duplicada — o branch isolado acima e um segundo ponto dentro do shell interno (`view === 'cliente'`, usado na busca multi-prestador) tinham ~700 linhas quase idênticas de JSX, e todo ajuste de layout precisava ser feito 2x (fonte de bugs simétricos ao longo de uma sessão inteira de patches). Unificado numa função só, `renderPublicSections(multiProvider)`, chamada nos dois pontos (`renderPublicSections(false)` no branch isolado, `renderPublicSections(true)` dentro do shell) — as diferenças de conteúdo entre os dois modos (campo de busca, nome do prestador no card, wrapper de tema `storefrontStyle-*` que só o modo `false` aplica) viraram ternários dentro da função, não mais dois blocos de código. **A partir de agora, mexer na página pública é só nesse um lugar** — não recriar a duplicação.

Junto da unificação, o modelo de agendamento virou "carrinho de e-commerce" de verdade (pedido explícito do Jefferson, várias rodadas de ajuste na mesma sessão):
- **Catálogo em grid**, não carrossel (`.providerList { display:grid; grid-template-columns: repeat(auto-fill, minmax(220px,1fr)) }`) — carrossel (`.publicGallery`/`.publicBanners`) ficou só pra banners/galeria da vitrine, não pra comparar preço/duração entre serviços.
- **Card de serviço vira produto**: imagem real ou placeholder consistente (`.cardImagePlaceholder`, ícone `Image` — antes cards sem foto simplesmente não tinham área de imagem, quebrando o ritmo visual de grade de produtos), preço em destaque separado da duração (`.priceTag`), botão real `.addServiceButton` (antes era um `::after` de CSS decorativo, só existia no tema premium).
- **Modelo "1 principal + N extras" tornado explícito, não abandonado**: `toggleServiceInBooking` (substituiu o antigo `toggleCartService`) adiciona/remove do mesmo jeito que um carrinho — primeiro serviço clicado vira `bookingForm.serviceId` (o que define o horário, badge "Define o horário" visível no card), os demais entram em `cartServiceIds` (itens extras, sem horário próprio, texto explícito no formulário: "Itens extras — sem horário próprio, combinados no mesmo atendimento"). Continua **sem** motor de agenda por item — `duration` continua só informativo (ver §10), decisão consciente pra não reescrever `createBooking`/`availableTimes` pra múltiplos horários numa submissão.
- **Carrinho flutuante** (`.floatingCartWrap`, canto superior direito, `position:fixed`) com contador — clicar abre um painel dropdown (`.cartPanel`) listando os itens com botão de remover cada um, e um botão "Ir para o agendamento" que leva pro checkout.
- **Checkout é uma tela própria**, não uma seção que aparece na mesma página rolando — `checkoutStep` (state) alterna entre mostrar a vitrine+catálogo OU só o formulário+botão "Voltar pra escolher serviços". Ao trocar de tela, o scroll pro topo é **sempre instantâneo** (`window.scrollTo(0, 0)`), nunca `behavior:'smooth'` — ver gotcha em §10 sobre por que isso importava de verdade.
- **Notificação "adicionado/removido à cotação"** (`.cartToast`, pop-up centralizado no topo, `pointer-events:none` — ver gotcha §10) some sozinha depois de 1.4s.

Seções reordenadas (banners logo após o hero, "Conheça o espaço" = só chips+galeria institucional, sem mais lista de serviços dentro dela).
...
- **Especificidade CSS pode deixar um clique "invisível" mesmo funcionando 100% por dentro (2026-08-30, achado depois de uma sessão inteira de debug remoto sem conseguir reproduzir)**: o Jefferson reportava "não consigo selecionar a hora, nada acontece" no modo escuro — testado exaustivamente (Chrome/Firefox, claro/escuro, mobile/desktop, clique real via mouse.down/up, `elementFromPoint`) e **tudo funcionava** nos testes automatizados. A causa real: `:root[data-theme='dark'] .timePicker button { background: var(--surface); ... }` (regra genérica de tema escuro, especificidade 0,3,1: `:root` + `[data-theme]` + `.timePicker` + `button`) tem **mais peso** que `.timePicker button.selected { background: var(--accent); ... }` (0,2,1: `.timePicker` + `button` + `.selected`) — CSS conta por número de seletores de classe/atributo, não por "quão específico o estado parece". Resultado: o clique registrava certinho (`bookingForm.time` mudava, `className` ganhava `selected`), mas a cor nunca trocava visualmente, então parecia que nada tinha acontecido. Corrigido com uma regra dedicada `:root[data-theme='dark'] .timePicker button.selected { background: var(--accent); ... }` (especificidade maior que a genérica). **Lição pra próxima vez que "clique não funciona" for reportado e os testes automatizados passarem**: checar se existe uma regra de tema (claro/escuro, ou qualquer variante de estilo) que reaplica `background`/`color` num seletor com MAIS classes/atributos que o estado (`.selected`/`.active`/etc.) que deveria vencer — especificidade, não a ordem no arquivo, decide o empate.
- **Scroll `behavior:'smooth'` ao trocar de tela pode causar cliques "perdidos" por alguns instantes**: quando a troca de tela reduz drasticamente a altura do documento (ex.: ir da vitrine longa pro checkout curto) e o scroll ainda está animando, o layout muda de posição embaixo do cursor por ~250-500ms — um clique nesse meio tempo cai em outro elemento. Trocar `window.scrollTo({top:0, behavior:'smooth'})` por `window.scrollTo(0, 0)` (instantâneo) nessas trocas de tela resolve; `behavior:'smooth'` continua ok pra scroll dentro da MESMA tela (ex.: rolar até uma seção existente sem trocar o que está montado).
...
**A rede de segurança não é 100% confiável (achado em 2026-08-30)**: apareceu mojibake bem visível em `App.jsx` (19 ocorrências de um traço `—` corrompido, mais `Área`/`Ícone` virando sequências longas tipo `ÃƒÆ'Ã‚Â¢...`) e `node scripts/fix-encoding.mjs` disse "nenhum mojibake encontrado — tudo em UTF-8 válido" mesmo assim — corrupção em camada demais (triplo-encode) parece passar da heurística de detecção do script. **Se `fix-encoding.mjs` disser que está tudo certo mas o navegador mostrar caractere estranho, não confiar cegamente** — conferir direto no arquivo (`grep -c "Ã" src/App.jsx` ou abrir e olhar). Corrigido manualmente nessa ocorrência via script Node pontual (achar o texto ASCII ao redor como âncora, recortar só o trecho corrompido no meio, escrever de volta com `fs.writeFileSync(path, conteudo, 'utf8')`) — mais confiável que tentar usar Edit/old_string quando o próprio terminal não consegue exibir/copiar os bytes corrompidos com fidelidade.
